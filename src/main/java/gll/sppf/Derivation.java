...
/**
 * A derivation node in the shared packed parse forest. Derivation nodes are
 * labeled with grammar slots or syntactic sorts and the input range they
 * describe. Derivation nodes are merged if they are labeled identically, that
 * is, if they are labeled with the same grammar slot or syntactic sort and the
 * same input range. Initially, derivation nodes point to zero or one packed
 * node, but after merging, derivation nodes can point to arbitrarily many
 * packed nodes.
 * 
